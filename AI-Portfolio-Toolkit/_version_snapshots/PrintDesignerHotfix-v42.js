...
// V42 hotfix: keep real project data even when profile placeholders are still empty.
(function () {
  'use strict';
  ...
  const isDemoData = data => {
    ...
    const hasProfile = ['name_th','name_en','role_th','role_en','bio_th','bio_en','skills_th','skills_en','email','phone','linkedin','avatar']
      .some(key => escText(data[key]) && !/^(Your Portfolio|Portfolio Template|Your Name)$/i.test(escText(data[key])));
