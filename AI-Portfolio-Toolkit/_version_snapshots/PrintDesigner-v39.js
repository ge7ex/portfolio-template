...
// V39 Interactive Print Designer - integrated data resolver + portrait-only responsive print studio
...
(function () {
  ...
  function injectV39Css() {
    if (document.getElementById('pd-v39-responsive-pagination-style')) return;
    const style = document.createElement('style');
    style.id = 'pd-v39-responsive-pagination-style';
    style.textContent = `
      html{-webkit-text-size-adjust:100%;text-size-adjust:100%}img,video,canvas,svg{max-width:100%;height:auto}body{overflow-x:hidden}#app{overflow-wrap:anywhere}
      @media(max-width:1023px){
        nav{left:max(10px,env(safe-area-inset-left))!important;right:max(10px,env(safe-area-inset-right))!important;top:max(10px,env(safe-area-inset-top))!important;transform:none!important;width:auto!important;max-width:calc(100vw - 20px)!important;overflow-x:auto!important;overflow-y:hidden!important;justify-content:flex-start!important;gap:.5rem!important;padding:.45rem!important;border-radius:1.25rem!important;scrollbar-width:none!important;-ms-overflow-style:none!important}
        nav::-webkit-scrollbar{display:none!important}nav button{flex:0 0 auto!important;min-width:44px!important;min-height:44px!important}
        ...
        #theme-panel{position:fixed!important;left:max(12px,env(safe-area-inset-left))!important;right:max(12px,env(safe-area-inset-right))!important;top:76px!important;width:auto!important;max-height:calc(100dvh - 92px)!important;overflow-y:auto!important}
        #public-export-branch{position:fixed!important;right:max(12px,env(safe-area-inset-right))!important;bottom:max(12px,env(safe-area-inset-bottom))!important;z-index:70!important}
        #edit-modal{padding:.75rem!important;align-items:stretch!important}#edit-modal>div{width:100%!important;max-width:100%!important;max-height:calc(100dvh - 1.5rem)!important;border-radius:1.25rem!important}
        #edit-modal .px-8{padding-left:1rem!important;padding-right:1rem!important}#edit-modal .p-8{padding:1rem!important}#edit-modal .gap-8{gap:.75rem!important}
        #edit-modal input,#edit-modal textarea,#edit-modal select{font-size:16px!important;min-width:0!important}.resume-cv-shell{grid-template-columns:1fr!important}.resume-cv-sidebar,.resume-cv-main{width:100%!important}
      }
      @media(max-width:640px){
        nav .px-4{padding-left:.8rem!important;padding-right:.8rem!important}nav button[onclick="toggleModal()"] span{display:none!important}
        .portfolio-hero{border-radius:1.5rem!important}.hero-title{font-size:clamp(2.25rem,14vw,4rem)!important;line-height:.95!important}.hero-frame{width:min(88vw,300px)!important;padding:10px!important}.unified-card{border-radius:1.5rem!important}.scrollytelling-track img{width:82vw!important;max-width:82vw!important;height:220px!important}
        .resume-cv-shell{grid-template-columns:1fr!important;border-radius:1.5rem!important}.resume-cv-sidebar,.resume-cv-main{padding:1.25rem!important}
      }
      @media(max-width:900px){
        .print-designer-root .pd-body{grid-template-columns:1fr!important}.print-designer-root .pd-sidebar{max-height:38dvh!important;border-right:0!important;border-bottom:1px solid rgba(148,163,184,.25)!important}.print-designer-root .pd-workspace{padding:12px!important}.print-designer-root .pd-stage{justify-content:flex-start!important;padding:12px!important}.print-designer-root .pd-page{transform-origin:top left!important}.print-designer-root .pd-topbar{height:auto!important;min-height:68px!important;align-items:flex-start!important}.print-designer-root .pd-top-actions{overflow-x:auto!important;flex-wrap:nowrap!important;max-width:100%!important}.print-designer-root .pd-top-actions button{flex:0 0 auto!important}
      }
      @media print{
        @page{size:A4 portrait;margin:0}
        h1,h2,h3,h4,h5,.pd-section-heading,.resume-cv-main h3,.resume-cv-sidebar h3{break-after:avoid-page!important;page-break-after:avoid!important;orphans:3!important;widows:3!important}
        p,li,.pd-project-desc,.pd-project-list{orphans:3!important;widows:3!important}
        .resume-card,.print-exp-item,.unified-card,.print-order-extra,.pd-project,.pd-image,.pd-object-content{break-inside:avoid-page!important;page-break-inside:avoid!important}
        .experience-section,.resume-cv-main,.resume-cv-sidebar,.print-layout-wrapper{break-inside:auto!important;page-break-inside:auto!important}
        .cinematic-image-wrapper,.cinematic-image-inner,.scrollytelling-wrapper,.scrollytelling-track{max-height:none!important;overflow:visible!important;opacity:1!important;transform:none!important}
        .studio-grid,.studio-guide,.studio-overlay{display:none!important}
      }
    `;
    document.head.appendChild(style);
  }

  function forcePortraitProfile(profile) {
    if (profile) profile.orientation = 'portrait';
    return profile;
  }
  ...
  function activeProfile() { const store = modeStore(); let p = store.profiles.find(x => x.id === store.activeProfileId) || store.profiles[0]; store.activeProfileId = p.id; p.transforms ||= {}; forcePortraitProfile(p); state.activeProfile = p; return p; }
  ...
  function getCandidateFromWindow() {
    const names = ['__AI_PORTFOLIO_ACTIVE_DATA', 'ACTIVE_PORTFOLIO_DATA', 'INJECTED_PORTFOLIO_DATA', 'currentData', 'portfolioData', 'profileData'];
  ...
  function dataFromRenderedPage() {
    ...
    const sections = Array.from(app.querySelectorAll('section, .print-order-extra, .resume-section-card, .unified-card'));
    ...
    const findSection = regex => sections.find(section => regex.test(meaningfulText(section.textContent).slice(0, 180)));
    const expRoot = app.querySelector('.experience-section') || findSection(/experience|ประสบการณ์|ผลงาน|project/i);
    if (expRoot) {
      const itemSelectors = '.print-exp-item, .scrollytelling-wrapper, .resume-card, .story-section, .unified-card';
      let items = Array.from(expRoot.querySelectorAll(itemSelectors));
      if (!items.length && /experience|ประสบการณ์|ผลงาน|project/i.test(meaningfulText(expRoot.textContent))) items = [expRoot];
      d.exp = items.slice(0, 12).map((item, index) => {
    ...
    if (!Array.isArray(d.exp) || !d.exp.length) {
      const images = Array.from(app.querySelectorAll('img')).map(imageSrc).filter(Boolean).filter(src => src !== avatar);
      if (images.length) d.exp = [{ title_th: 'Project Images', title_en: 'Project Images', company_th: '', company_en: '', desc_th: '', desc_en: '', images }];
    }

    d.bio_th ||= sectionText([/summary|ประวัติโดยย่อ|professional/i], [/^ประวัติโดยย่อ/i, /^Professional Summary/i, /^Summary/i]);
    d.bio_en ||= d.bio_th;
    d.skills_th ||= sectionText([/skills|ทักษะ/i], [/^ทักษะ(และความเชี่ยวชาญ)?/i, /^Skills( & Expertise)?/i]);
    d.skills_en ||= d.skills_th;
  ...
  function bindUi() {
    ...
    document.getElementById('pd-orientation').onchange = () => { activeProfile().orientation = 'portrait'; render(); };
  ...
  function applyPage() {
    ...
    forcePortraitProfile(p);
    page.className = `pd-page pd-${state.mode} pd-portrait ${p.grid ? 'pd-grid-on' : ''}`;
  ...
  function baseBoxes() {
    ...
    out.push({ id: 'header', type: 'text', x: margin, y: margin, w: d.avatar ? contentW - 48 : contentW, h: 34, minH: 34, z: 3, html: `<h1 style="color:#fff">${esc(t.name || 'Your Name')}</h1><h3 style="color:#dbeafe">${esc(t.role || '')}</h3>` });
  ...
  function open(mode) { shell(); ensureViewportMeta(); injectV39Css(); state.data = resolvePrintData(); state.profiles = loadProfiles(); state.mode = mode === 'resume' || mode === 'portfolio' ? mode : (state.data.layout || 'portfolio'); activeProfile(); document.getElementById('print-designer-root').classList.remove('hidden'); document.body.classList.add('print-designer-open'); render(); }
