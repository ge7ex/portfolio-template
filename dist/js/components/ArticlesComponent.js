...
const ArticlesComponent={render:(content,layout,colorMode,pStyle,lang)=>{const items=_lines(content);if(!items.length)return'';const t=_theme(layout,colorMode,pStyle);const title=lang==='th'?'บทความ / สื่อเผยแพร่':'Articles / Publications';const body=items.map(l=>{const [name,desc,url]=_parts(l);return `<div class="extra-card compact"><h4>${url?`<a href="${url}" target="_blank" rel="noopener noreferrer">${name||''}</a>`:name||''}</h4>${desc?`<p>${desc}</p>`:''}</div>`}).join('');return _wrap(_title(title,t)+`<div class="extra-grid">${body}</div>`,'articles-section')}};
