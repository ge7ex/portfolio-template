...
const ServicesComponent={render:(content,layout,colorMode,pStyle,lang)=>{const items=_lines(content);if(!items.length)return'';const t=_theme(layout,colorMode,pStyle);const title=lang==='th'?'บริการ / สิ่งที่ช่วยได้':'Services';const body=items.map(l=>{const [name,desc]=_parts(l);return `<div class="extra-card compact"><h4>${name||''}</h4>${desc?`<p>${desc}</p>`:''}</div>`}).join('');return _wrap(_title(title,t)+`<div class="extra-grid three">${body}</div>`,'services-section')}};
